...
The Codex target includes `.codex/config.example.toml` as a safe project-local example. Keep machine trust, live `~/.codex` state, auth, sessions, SQLite databases, and plugin caches out of product repos. See [`docs/codex-machine-policy.md`](docs/codex-machine-policy.md) and [`docs/untrusted-repo-intake.md`](docs/untrusted-repo-intake.md).
